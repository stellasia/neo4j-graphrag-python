...
class Orchestrator:
    ...
    async def run_task(self, task: TaskPipelineNode, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            return None
        self.event_notifier.notify_task_started(self.run_id, task.name, inputs)
        res = await task.run(inputs)
        await self.set_task_status(task.name, RunStatus.DONE)
        self.event_notifier.notify_task_finished(self.run_id, task.name, res)
        if res:
    ...
    async def run(self, data: dict[str, Any]) -> None:
        ...
        """
        self.event_notifier.notify_pipeline_started(self.run_id, data)
        tasks = [self.run_task(root, data) for root in self.pipeline.roots()]
        await asyncio.gather(*tasks)
        self.event_notifier.notify_pipeline_finished(
